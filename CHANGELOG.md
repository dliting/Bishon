...
- **`make-release.sh --skip-pyenv --skip-models --skip-image`** (was `--skip-env`, renamed in v2.2.0): when all three skips are passed, produces a tiny source-only tarball in ~5s for quick publish testing. Useful for iterating on bundle layout without waiting on the 12 GB env/models/image copy.
