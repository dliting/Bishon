...
# /opt/bishon-home/bishon/docker/entrypoint.sh (bind-mounted from host-dir)
...
REAL=/opt/bishon-home/bishon/docker/entrypoint.sh

if [ ! -x "$REAL" ]; then
    echo "[launcher] FATAL: $REAL missing or not executable." >&2
    echo "[launcher]        Run install.sh to populate /opt/bishon-home/ first." >&2
    exit 1
fi
