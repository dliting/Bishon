...
// Conversation history (context for the next turn).
const history = ref([]);

// Whether an answer is currently in progress.
const showLoading = ref(false);

// Used to abort the in-flight request.
...
const like = useThrottleFn((item, e) => {
  item.like = !item.like;
  item.unlike = false;
  _czc.push(['_trackEvent', 'bishon', '问答页面', '点赞', '', '']);
  if (item.like) {
...
const unlike = (item: IChatItem) => {
  ...
  item.like = false;
  _czc.push(['_trackEvent', 'bishon', '问答页面', '点踩', '', '']);
};
...
// Download and clear-chat-related state.
...
.sourceitem-leave,   // before leave / after enter: opacity = 1
.sourceitem-enter-to {
  opacity: 1;
}
.sourceitem-leave-active,
.sourceitem-enter-active {
  transition: opacity 0.5s; // transition duration: 0.5s
}
.sourceitem-leave-to,
.sourceitem-enter {
  opacity: 0;
