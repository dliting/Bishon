...
// Whether the confirm button can submit.
...
// Whether the knowledge base name is editable.
...
// Knowledge base ID after creation.
const newId = ref('');
...
// Hide the upload panel until the KB is created (the backend requires a KB to exist before uploads).
const showUpload = computed(() => {
  return (newId.value && modalTitle.value === '新建知识库') || modalTitle.value !== '新建知识库';
});

// Whether to show the upload file list (hidden by default).
...
const uplolad = () => {
  ...
  fileList.value.forEach(async (file: IFileListItem, index) => {
    if (file.status == 'loading') {
      try {
        // Upload mode: 'soft' = skip duplicate file names, 'strong' = overwrite duplicate file names.
...
const deleteFile = async (item: IFileListItem, index: number) => {
  ...
  if (item.file_id) {
    ...
    // Failed uploads have no fileId; remove directly from the file list.
    fileList.value.splice(index, 1);
    message.success('删除成功');
  }
};
...
const addKnowledge = async () => {
  ...
  // After receiving the kb_id, assign it to newId.
  try {
